				    ...
				    unsigned int sample_idx)
{
	...
	dac_write(SPI_CS_DAC,
		  buffer->values_current[sample_idx] | DAC_CH_A_ADDR);
	/* write the emulation current value from buffer to DAC */
	dac_write(SPI_CS_DAC,
		  buffer->values_voltage[sample_idx] | DAC_CH_V_ADDR);
...
void sampling_init(enum ShepherdMode mode, unsigned int harvesting_voltage)
{
	...
	adc_readwrite(SPI_CS_ADC, ((ADC_CH_V_IN + 5) << 9) | (1 << 8) |
					  (1 << 2) | (1 << 1));
	adc_readwrite(SPI_CS_ADC, ((ADC_CH_V_OUT + 5) << 9) | (1 << 8) |
					  (1 << 2) | (1 << 1));

	/* Range +/-0.625*VREF */
	adc_readwrite(SPI_CS_ADC,
		      ((ADC_CH_A_IN + 5) << 9) | (1 << 8) | (1 << 1));
	adc_readwrite(SPI_CS_ADC,
		      ((ADC_CH_A_OUT + 5) << 9) | (1 << 8) | (1 << 1));
