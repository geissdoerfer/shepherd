...
volatile struct SharedMem * shared_mem = (volatile struct SharedMem *) PRU_SHARED_MEM_STRUCT_OFFSET;

enum SyncState {IDLE, WAIT_IEP_WRAP, WAIT_HOST_INT, REQUEST_PENDING, REPLY_PENDING};
...
static inline int check_gpio(volatile struct SharedMem * shared_mem)
{
	...
	if(diff > 0) {
		...
		shared_mem->gpio_edges->ticks_offset[shared_mem->gpio_edges->idx] = CT_IEP.TMR_CNT;
		shared_mem->gpio_edges->bitmask[shared_mem->gpio_edges->idx] = (char) now_status;
...
void event_loop(volatile struct SharedMem * shared_mem)
{
	...
	struct msg_ctrl_rep_s * ctrl_rep = (struct msg_ctrl_rep_s *) &rpmsg_buffer;
	...
	while(1)
	{
		...
		if(iep_check_evt_cmp(IEP_CMP1) == 0) {
			...
			if(++sample_counter==SAMPLES_PER_BUFFER) {
				...
				unsigned int next_cmp_val = iep_get_cmp_val(IEP_CMP1) + sample_period;
			...
			if(sync_state == REPLY_PENDING) {
				if( check_control_reply(ctrl_rep) == 0 ) {
					...
					unsigned int block_period = TIMER_BASE_PERIOD + ctrl_rep->clock_corr;
				...
				rpmsg_putraw(&ctrl_req, sizeof(struct msg_ctrl_req_s));
				_GPIO_TOGGLE(P8_41);
